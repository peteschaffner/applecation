(function($, undefined) {
	...
	function initMap() {
		...
		// Add the circle overlays
		for (var i = 0; i < miles_radii.length; i++) {
			circles.push(new google.maps.Circle({
				...
				map: map,
				center: cityCenter,
				radius: miles_radii[i] * 1609.34 /* miles to meters conversion */
			}));
		}
				...
				placesService.nearbySearch(request, function(results, status, pagination) {
					if (status == google.maps.places.PlacesServiceStatus.OK) {
						for (var i = 0; i < results.length; i++) {
							var place = results[i];
							var placeObj = {};
							placeObj[place.reference] = place;
	...
	function addResult(place, rank) {
		var type = place.types[0],
			category = (placeTypes[type] === undefined ? false : placeTypes[type]),
			reference = place.reference;

		if (category === false || searchResults.places[reference] !== undefined) {return false;}
		...
		searchResults.places[place.reference] = {
			marker: marker,
			rank: rank,
			category: category
		}
	...
	function clearSearchResults() {
		...
		$.each(searchResults.places, function(reference, place){
